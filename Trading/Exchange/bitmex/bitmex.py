...
from octobot_trading.enums import ExchangeConstantsFundingColumns, \
    ExchangeConstantsMarkPriceColumns, PositionStatus, PositionSide, ExchangeConstantsOrderBookTickerColumns
...
class Bitmex(FutureExchange):
    ...
    # Order Book ticker
    BITMEX_ASK_QUANTITY = "askSize"
    BITMEX_ASK_PRICE = "askPrice"
    BITMEX_BID_QUANTITY = "bidSize"
    BITMEX_BID_PRICE = "bidPrice"
    ...
    def parse_order_book_ticker(self, order_book_ticker):
        try:
            order_book_ticker.update({
                ExchangeConstantsOrderBookTickerColumns.ASK_QUANTITY.value:
                    self.client.safe_float(order_book_ticker, self.BITMEX_ASK_QUANTITY, 0),
                ExchangeConstantsOrderBookTickerColumns.ASK_PRICE.value:
                    self.client.safe_float(order_book_ticker, self.BITMEX_ASK_PRICE, 0),
                ExchangeConstantsOrderBookTickerColumns.BID_QUANTITY.value:
                    self.client.safe_float(order_book_ticker, self.BITMEX_BID_QUANTITY, 0),
                ExchangeConstantsOrderBookTickerColumns.BID_PRICE.value:
                    self.client.safe_float(order_book_ticker, self.BITMEX_BID_PRICE, 0)
            })
        except KeyError as e:
            self.logger.error(f"Fail to parse order book ticker dict ({e})")
