...
from octobot_trading.constants import RECENT_TRADES_CHANNEL, ORDERS_CHANNEL, BALANCE_CHANNEL, \
    ORDER_BOOK_CHANNEL, TICKER_CHANNEL, MARK_PRICE_CHANNEL, FUNDING_CHANNEL, POSITIONS_CHANNEL, LIQUIDATIONS_CHANNEL, \
...
class Bitmex(WebsocketExchange):
    ...
    async def _book_ticker(self, msg: dict):
        if msg['data'] and msg['data'][0]:
            book_ticker: dict = self.exchange.parse_order_book_ticker(msg['data'][0])
            await self.push_to_channel(ORDER_BOOK_TICKER_CHANNEL,
                                       symbol=book_ticker[ExchangeConstantsOrderBookTickerColumns.SYMBOL.value],
                                       ask_quantity=book_ticker[ExchangeConstantsOrderBookTickerColumns.ASK_QUANTITY.value],
                                       ask_price=book_ticker[ExchangeConstantsOrderBookTickerColumns.ASK_PRICE.value],
                                       bid_quantity=book_ticker[ExchangeConstantsOrderBookTickerColumns.BID_QUANTITY.value],
                                       bid_price=book_ticker[ExchangeConstantsOrderBookTickerColumns.BID_PRICE.value])
    ...
    async def _position(self, msg: dict, action: str):
        if msg['data']:
            if action == 'partial':
                self.positions = {position['symbol']: position for position in msg['data']}
            else:
                for position in msg['data']:
                    self.positions[position['symbol']].update(position)
            await self.push_to_channel(POSITIONS_CHANNEL,
                                       positions=[self.exchange.parse_position(position)
    ...
    async def _liquidation(self, msg: dict):
        if msg['data']:
            symbol = self.get_pair_from_exchange(msg['data'][0]['symbol'])
            await self.push_to_channel(LIQUIDATIONS_CHANNEL,
                                       symbol=symbol,
                                       liquidations=[self.exchange.parse_liquidation(liquidation)
    ...
    async def _handle_action(self, action: str, table: str, parsed_message: dict):
        if table == self.EXCHANGE_FEEDS[Feeds.TRADES]:
        ...
        elif table == self.EXCHANGE_FEEDS[Feeds.BOOK_TICKER]:
            await self._book_ticker(parsed_message)
            ...
            await self._position(parsed_message, action)
        ...
        elif table == self.EXCHANGE_FEEDS[Feeds.LIQUIDATIONS]:
            await self._liquidation(parsed_message)
        else:
