...
from octobot_trading.enums import TraderOrderType, TradeOrderSide, OrderStatus, EvaluatorStates, \
...
class DipAnalyserTradingMode(AbstractTradingMode):
    ...
    async def create_producers(self) -> list:
        mode_producer = DipAnalyserTradingModeProducer(get_chan(MODE_CHANNEL, self.exchange_manager.id),
...
class DipAnalyserTradingModeConsumer(AbstractTradingModeConsumer):
    ...
    async def create_new_orders(self, symbol, final_note, state, **kwargs):
        ...
        if state == EvaluatorStates.LONG.value:
    ...
    async def create_buy_order(self, symbol, timeout, volume_weight, price_weight):
        ...
        try:
            ...
            limit_price = adapt_price(symbol_market, self.get_limit_price(price))
...
class DipAnalyserTradingModeProducer(AbstractTradingModeProducer):
    ...
    def __init__(self, channel, config, trading_mode, exchange_manager):
        ...
        self.base, _ = split_symbol(self.trading_mode.symbol)
    ...
    async def set_final_eval(self, matrix_id: str, cryptocurrency: str, symbol: str, time_frame):
        ...
        for evaluated_strategy_node in get_tentacles_value_nodes(
                ...
                get_tentacle_nodes(matrix_id,
                                   ...
                                   tentacle_name=DipAnalyserStrategyEvaluator.get_name()),
                symbol=symbol):
            if check_valid_eval_note(get_value(evaluated_strategy_node),
                                     get_type(evaluated_strategy_node),
                                     DipAnalyserStrategyEvaluator.get_eval_type()):
                self.final_eval = get_value(evaluated_strategy_node)
                await self.create_state()

    async def create_state(self):
        self.state = EvaluatorStates.LONG
    ...
    async def order_filled_callback(self, filled_order):
        if filled_order[ExchangeConstantsOrderColumns.SIDE.value] == TradeOrderSide.BUY.value:
            self.state = EvaluatorStates.SHORT
            paid_fees = total_fees_from_order_dict(filled_order, self.base)
            sell_quantity = filled_order[ExchangeConstantsOrderColumns.FILLED.value] - paid_fees
            await self._create_sell_order_if_enabled(filled_order[ExchangeConstantsOrderColumns.ID.value],
                                                     sell_quantity,
                                                     filled_order[ExchangeConstantsOrderColumns.PRICE.value])
